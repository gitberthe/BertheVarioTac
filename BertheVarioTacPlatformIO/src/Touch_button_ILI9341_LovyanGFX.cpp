...
// cp ./src/User_Setup.h ./.pio/libdeps/esp32dev/TFT_eSPI/User_Setup.h
...
public:
  LGFX(void)
  {
    ...
    { // Set the display panel control.//表示パネル制御の設定を行います。
      ...
      cfg.memory_width     =   320;  // Maximum width supported by driver IC // ドライバICがサポートしている最大の幅
      cfg.memory_height    =   240;  // Maximum height supported by driver IC // ドライバICがサポートしている最大の高さ
      cfg.panel_width      =   320;  // Actually displayable width // 実際に表示可能な幅
      cfg.panel_height     =   240;  // Actually displayable height // 実際に表示可能な高さ
      ...
      cfg.rgb_order        = true;  // Set to true if the red and blue of the panel are swapped // パネルの赤と青が入れ替わってしまう場合 trueに設定
