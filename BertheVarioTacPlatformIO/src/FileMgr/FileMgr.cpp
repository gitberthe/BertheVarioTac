...
ESPFMfGK * g_pfilemgr = NULL ; // (filemanagerport);
...
void addFileSystems()
{
  ...
  */
  ...
  //if (SD.begin(SS))
  //{ */
    if (! g_pfilemgr->AddFS(SD, "SD-Card", false)) {
...
{
  // See above.
  g_pfilemgr->checkFileFlags = checkFileFlags;

  g_pfilemgr->WebPageTitle = "FileManager";
  g_pfilemgr->BackgroundColor = "white";
  g_pfilemgr->textareaCharset = "accept-charset=\"utf-8\"";

  if ((WiFi.status() == WL_CONNECTED) && (g_pfilemgr->begin())) {
